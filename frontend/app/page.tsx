...
import { PartSearchAutocomplete } from '@/components/ui/PartSearchAutocomplete';
...
const IconPrint = ({ size = 14 }: { size?: number }) => <svg width={size} height={size} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M6 9V2h12v7" /><path d="M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2" /><rect x="6" y="14" width="12" height="8" /></svg>;
...
export default function Home() {
  ...
  const doSearchParts = async () => {
    ...
    setPartLoading(true);
    try {
      // Use the restored Supabase direct search action
      const data = await searchSpareParts(selectedModel, '', false);
      // Since searchSpareParts does filtering internally if query is provided,
      // but here we might want all parts for a section or smart search.
      // If we are in section mode, we still need the section filter.
      let filtered = data;
      if (partSearchMode === 'section' && partSection) {
        filtered = data.filter(p => p.section_name === partSection);
      }
      setPartResults(filtered as unknown as Part[]);
    } catch {
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg-base)' }}>
      ...
      <header
        style={{
          position: 'sticky', top: 0, zIndex: 20, background: 'rgba(6,6,15,0.9)', backdropFilter: 'blur(12px)',
          borderBottom: '1px solid var(--border-subtle)', padding: '12px 24px', display: 'flex', alignItems: 'center', gap: 16,
        }}
      >
        <button
          type="button" className="btn btn-ghost" style={{ padding: '8px 14px', gap: 6 }}
          onClick={() => { setExpandedSection(null); setSelectedModel(''); setPartResults([]); }}
        >
          <IcoBack /> Home
        </button>
        <div style={{ height: 20, width: 1, background: 'var(--border-subtle)' }} />
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ color: 'var(--red)', display: 'flex' }}><IcoWrench size={18} /></span>
          <span style={{ fontWeight: 700, fontSize: '0.9rem' }}>Catalogo Parti</span>
        </div>
        <div style={{ flex: 1 }} />
        <button
          type="button" className="btn"
          style={{
            background: isCartOpen ? 'var(--bg-elevated)' : 'transparent',
            border: '1px solid var(--border-default)', fontSize: '0.82rem', padding: '7px 14px', gap: 6,
          }}
          onClick={() => setIsCartOpen(true)}
        >
          <IconCart /> Preferiti {cart.length > 0 && <span className="badge badge-orange">{cart.length}</span>}
        </button>
      </header>

      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 24px' }}>
        {/* Model and Mode selector */}
        <div className="surface" style={{ padding: '20px', marginBottom: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16, marginBottom: 16 }}>
            <ModelSelect
              printers={printers}
              value={selectedModel}
              onChange={(v) => { setSelectedModel(v); setPartResults([]); setPartSection(''); }}
              placeholder="Seleziona modello per catalogo…"
            />
            <div className="toggle-group">
              <button
                className={`toggle-btn ${partSearchMode === 'smart' ? 'active' : ''}`}
                onClick={() => { setPartSearchMode('smart'); setPartResults([]); }}
              >
                Smart Search
              </button>
              <button
                className={`toggle-btn ${partSearchMode === 'section' ? 'active' : ''}`}
                onClick={() => { setPartSearchMode('section'); setPartResults([]); }}
              >
                Per Sezione
              </button>
            </div>
          ...
          {partSearchMode === 'smart' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 10 }}>
              <PartSearchAutocomplete
                model={selectedModel}
                onSelect={(p) => { if (p) addToCart(p); }}
                onResults={setPartResults}
                placeholder="Cerca per codice o descrizione ricambio…"
              />
            </div>
          )}
          ...
          {partSearchMode === 'section' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 12 }}>
              <div style={{ position: 'relative', flex: 1 }}>
                <svg
                  style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 16, height: 16, color: 'var(--text-secondary)', pointerEvents: 'none' }}
                ...
                >
                  ...
                  {partSectionsList.map((s) => (<option key={s} value={s}>{s}</option>))}
                </select>
                <svg
                  style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', width: 14, height: 14, color: 'var(--text-secondary)', pointerEvents: 'none' }}
            ...
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontWeight: 700, fontSize: '0.88rem' }}>
                  Risultati <span className="chip chip-neutral" style={{ marginLeft: 6 }}>{partResults.length}</span>
                </span>
                {partResults.length > 0 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 20 }}>
                    ...
                    >
                      ...
                      {Array.from(new Set(partResults.map(p => p.model))).sort().map(m => (<option key={m} value={m}>{m}</option>))}
                    </select>
                  </div>
                )}
              </div>
                ...
                <thead style={{ position: 'sticky', top: 0, background: 'var(--bg-elevated)', zIndex: 5 }}>
                  <tr>
                    {['Pg', 'Ref', 'Modello', 'Codice', 'Descrizione', 'Qty', 'Sezione', '♥'].map(h => (
                      <th key={h} style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', color: 'var(--text-secondary)', borderBottom: '1px solid var(--border-subtle)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {partLoading ? (
                    ...
                    (selectedModelFilter ? partResults.filter(p => p.model === selectedModelFilter) : partResults).map((part, idx) => {
                      const uid = uniqueId(part);
                      const isFav = cart.some(c => uniqueId(c) === uid);
                      return (
                        <tr key={uid || idx} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                          ...
                          <td style={{ padding: '9px 12px' }}><span className="chip chip-blue" style={{ fontSize: '0.68rem' }}>{part.model}</span></td>
                          <td style={{ padding: '9px 12px' }}><span className="mono" style={{ color: 'var(--accent)', fontWeight: 700 }}>{part.part_code}</span></td>
                          <td style={{ padding: '9px 12px', maxWidth: 300 }}>{part.name}</td>
                          <td style={{ padding: '9px 12px' }}>{part.quantity}</td>
                          <td style={{ padding: '9px 12px' }}><span className="chip chip-neutral" style={{ fontSize: '0.68rem' }}>{part.section_name}</span></td>
                          <td style={{ padding: '9px 12px' }}>
                            <button className="btn-icon" onClick={() => isFav ? removeFromCart(cart.find(c => uniqueId(c) === uid)!.cartId) : addToCart(part)} style={{ color: isFav ? 'var(--red)' : 'var(--text-muted)' }}>
        ...
        {/* Empty state */}
        {!partLoading && partResults.length === 0 && selectedModel && (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-muted)' }}>
            Seleziona una sezione o usa la Smart Search per trovare ricambi.
          </div>
        )}
      </div>
      ...
      {isCartOpen && (
        <div className="drawer-overlay" onClick={() => setIsCartOpen(false)}>
          <div className="drawer animate-slide-l" onClick={e => e.stopPropagation()}>
            <div className="drawer-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <IconCart /> <span style={{ fontWeight: 800 }}>I Miei Preferiti</span>
                <span className="chip chip-neutral">{cart.length}</span>
              </div>
              <button className="btn-icon" onClick={() => setIsCartOpen(false)}>✕</button>
            </div>

            <div className="drawer-content">
              {cart.length === 0 ? (
                <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>Lista vuota</div>
              ) : (
                cart.map(item => (
                  <div key={item.cartId} className="cart-item">
                    <div style={{ flex: 1 }}>
                      <div className="mono" style={{ color: 'var(--accent)', fontWeight: 700, fontSize: '0.85rem' }}>{item.part_code}</div>
                      <div style={{ fontSize: '0.85rem', marginTop: 2 }}>{item.name}</div>
                      <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: 4 }}>{item.model} · {item.section_name}</div>
                    </div>
                    <button className="btn-icon text-red" onClick={() => removeFromCart(item.cartId)}><IcoTrash /></button>
                  </div>
                ))
              )}
            </div>
            ...
            <div className="drawer-footer">
              <button className="btn btn-primary" style={{ width: '100%' }} onClick={copyCart} disabled={cart.length === 0}>
                <IconCopy /> Copia Lista
              </button>
              <button className="btn btn-ghost" style={{ width: '100%' }} onClick={printCart} disabled={cart.length === 0}>
                <IconPrint /> Stampa Lista
              </button>
            </div>
          </div>
        </div>
