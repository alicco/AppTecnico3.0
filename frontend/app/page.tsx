...
const IconPrinter = () => <svg width="26" height="26" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2m-6 0H8m4 0v4m0 0H8m4 0h4" /></svg>;
const IconAlert  = () => <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" /></svg>;
...
function ErrorsAccordion({ printers, favorites, onToggleFav, isOpen }: {
  ...
  return (
    <div className="accordion-body animate-expand">
      ...
      {searched && !isPending && results.length === 0 && <div className="empty-state"><div className="empty-icon" style={{ color: 'var(--text-dim)' }}><IconSearch /></div><p>Nessun risultato</p></div>}
      {results.length > 0 && (
...
function PartsAccordion({ printers, cart, onAddToCart, isOpen }: {
  ...
  return (
    <div className="accordion-body animate-expand">
      ...
      {searched && !isPending && results.length === 0 && <div className="empty-state"><div className="empty-icon" style={{ color: 'var(--text-dim)' }}><IconBox /></div><p>Nessun ricambio trovato</p></div>}
      {results.length > 0 && (
        <div style={{ marginTop: 16 }}>
          <div className="row-between" style={{ marginBottom: 10 }}>
    ...
    <div className="accordion-body animate-expand">
      <div className="viewfinder" style={{ position: 'relative' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ marginBottom: 8, opacity: 0.35, color: 'var(--accent-lcd)' }}>
...
function HomeView({ printers, favorites, cart, onToggleFav, onAddToCart, onNavigateWithCode }: {
  ...
  const panels: { id: AccordionPanel; label: string; sub: string; led: string; icon: React.ReactNode }[] = [
    { id: 'errors', label: 'Codici Errore',   sub: 'Diagnostica guasti',  led: 'led-orange', icon: <IconAlert /> },
    { id: 'parts',  label: 'Ricerca Ricambi', sub: 'Catalogo componenti', led: 'led-blue',   icon: <IconSettings /> },
    { id: 'scan',   label: 'Scanner Codice',  sub: 'Barcode & manuale',   led: 'led-green',  icon: <IconScan /> },
  ];

  return (
    <div>
      <div className="metal-panel" style={{ padding: '20px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 16 }}>
        <div style={{ width: 56, height: 56, borderRadius: 14, flexShrink: 0, background: 'linear-gradient(145deg, #252c3a 0%, #161c28 100%)', border: '1px solid rgba(255,255,255,0.10)', borderTopColor: 'rgba(255,255,255,0.18)', boxShadow: 'var(--shadow-raised)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent-orange)' }}>
          <IconPrinter />
        </div>
        <div>
          ...
          return (
            <div key={panel.id} className={`accordion-panel${isOpen ? ' expanded' : ''}`}>
              <div className="accordion-header" onClick={() => toggle(panel.id)}>
                ...
                <div style={{ color: isOpen ? 'var(--text-primary)' : 'var(--text-muted)' }}>{panel.icon}</div>
                <div style={{ flex: 1 }}>
...
function ErrorsView({ printers, favorites, onToggleFav }: { printers: Printer[]; favorites: FavItem[]; onToggleFav: (e: ErrorCode, name: string) => void; }) {
  ...
  return (
    <div>
      ...
      {searched && !isPending && results.length === 0 && <div className="empty-state"><div className="empty-icon" style={{ color: 'var(--text-dim)' }}><IconSearch /></div><p>Nessun risultato trovato</p></div>}
      ...
      <div className="metal-panel-deep" style={{ padding: 24, textAlign: 'center', marginBottom: 20 }}>
        ...
        <div className="viewfinder" style={{ position: 'relative', maxWidth: 320, margin: '0 auto 20px' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 11, color: 'rgba(0,255,157,0.35)', fontFamily: 'Rajdhani, sans-serif', letterSpacing: '1.2px', textTransform: 'uppercase' }}>Fotocamera non attiva</p>
          </div>
    ...
    <div className="drawer-overlay" onClick={onClose}>
      <div className="drawer" onClick={e => e.stopPropagation()}>
        ...
        <div className="row-between" style={{ marginBottom: 20 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, fontFamily: 'Rajdhani, sans-serif', letterSpacing: '0.5px', display: 'flex', alignItems: 'center', gap: 8 }}><IconCart /> Carrello</h2>
          <span className="badge badge-orange">{cart.length} art.</span>
        </div>
        {cart.length === 0 ? (
          <div className="empty-state"><div className="empty-icon" style={{ color: 'var(--text-dim)' }}><IconCart /></div><p>Carrello vuoto</p></div>
        ) : (
          <>
  ...
  return (
    <div className="drawer-overlay" onClick={onClose}>
      <div className="drawer" onClick={e => e.stopPropagation()}>
        <div className="drawer-handle" />
        <div className="row-between" style={{ marginBottom: 20 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, fontFamily: 'Rajdhani, sans-serif', letterSpacing: '0.5px', display: 'flex', alignItems: 'center', gap: 8 }}><IconStar /> Preferiti</h2>
          <span className="badge badge-orange">{favorites.length}</span>
        </div>
        {favorites.length === 0 ? (
          <div className="empty-state"><div className="empty-icon" style={{ color: 'var(--text-dim)' }}><IconStar /></div><p>Nessun preferito</p></div>
        ) : favorites.map(f => (
          <div key={f.id} className="result-card" style={{ marginBottom: 8 }}>
            <div className="row-between">
              <div><span className="error-code-big" style={{ fontSize: 16 }}>{f.code}</span><p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{f.printer}</p></div>
              <button onClick={() => onRemove(f.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 20, padding: 4 }}>×</button>
            </div>
          </div>
        ))}
      </div>
...
export default function AppTecnico() {
  ...
  function navigateWithCode(tab: Tab, _code: string) { setActiveTab(tab); }
    ...
    <>
      <header className="app-header">
        <div className="app-logo">
          <div className="app-logo-icon" style={{ color: 'var(--accent-orange)' }}>
            <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.6}><path strokeLinecap="round" strokeLinejoin="round" d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2m-6 0H8m4 0v4m0 0H8m4 0h4" /></svg>
          </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="header-btn" onClick={() => setShowFavs(true)} aria-label="Preferiti" style={{ position: 'relative' }}>
            <IconStar />{favorites.length > 0 && <span className="fav-count">{favorites.length}</span>}
          </button>
